...
import com.horizonloop.app.core.data.DialogueStorage
import com.horizonloop.app.core.data.parseTimeToSeconds
import com.horizonloop.app.features.player.data.AudioPlayer
import com.horizonloop.app.features.media.data.VideoScanner
import com.horizonloop.app.features.translation.data.TranslationWorkflow
...
class AppViewModel : ViewModel() {
    ...
    fun openPlayer(context: Context, audio: Audio) {
        ...
        translationSteps = emptyList(); translationLog = emptyList()
        translatedDialogues = DialogueStorage.loadDialogues(context, currentAudioFilePath)
    }

    fun goHome() { showHomeView = true; isPlaying = false; previewEndTime = null; audioPlayer?.pause(); if (audioMode) audioMode = false }
    ...
    fun seekTo(percent: Float) {
        ...
        // The AudioPlayer now pushes the new position via onProgressUpdate
        // immediately after seekTo(), so the UI updates in both playing and
        // paused states. No manual calculation needed here.
    }
    ...
    fun addNote(text: String) {
        val now = java.text.SimpleDateFormat("d/M/yyyy", java.util.Locale.getDefault()).format(java.util.Date())
        notes = listOf(Note(id = System.currentTimeMillis().toInt(), text = text, date = now)) + notes
    }
    ...
    fun startTranslation(context: Context) {
        ...
        val transcriptionModel = ApiKeyStorage.getTranscriptionModel(context)
        val translationModel = ApiKeyStorage.getEngine(context)
        TranslationWorkflow(
            context = context, audioFilePath = currentAudioFilePath, apiKey = apiKey,
            transcriptionModel = transcriptionModel, translationModel = translationModel,
            ...
            onDialoguesUpdate = { diag ->
                translatedDialogues = diag
                DialogueStorage.saveDialogues(context, currentAudioFilePath, diag)
            }, onTranslatingUpdate = { isTranslating = it },
            onShowDebugToggle = { showTranslationDebug = it }
        ).run(viewModelScope)
    }

    fun updateApiKey(context: Context) {}

    fun selectDialogue(context: Context, dialogue: Dialogue) {
        selectedDialogueIds = if (dialogue.id in selectedDialogueIds) selectedDialogueIds - dialogue.id else selectedDialogueIds + dialogue.id
        if (activeTab == ActiveTab.SAVE && dialogue.startTime >= 0) {
            audioPlayer?.seekTo((dialogue.startTime * 1000).toLong()); audioPlayer?.play(); isPlaying = true
        }
    }
    ...
    fun getCurrentDialogue(): Dialogue? {
        ...
        if (currentPlaybackTime < list[0].startTime) return list[0]
        var current: Dialogue? = null
        for (d in list) { if (d.startTime <= currentPlaybackTime) current = d else break }
        return current
    }

    fun getProgressPercent(): Float = if (totalDuration > 0) (currentPlaybackTime / totalDuration).toFloat() else 0f

    private fun skipDialogue(delta: Int) {
        val list = translatedDialogues
        if (list.isEmpty()) return
        val current = getCurrentDialogue()
        val currentIndex = list.indexOf(current)
        val nextIndex = when {
            delta > 0 -> if (currentIndex < 0 || currentIndex >= list.size - 1) 0 else currentIndex + 1
            else -> if (currentIndex <= 0) list.size - 1 else currentIndex - 1
        }
        val target = list[nextIndex]
        audioPlayer?.seekTo((target.startTime * 1000).toLong()); audioPlayer?.play(); isPlaying = true
    }
    fun skipToNextDialogue() = skipDialogue(+1)
    fun skipToPrevDialogue() = skipDialogue(-1)
