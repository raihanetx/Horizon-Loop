...
/**
 * Handles the full translation workflow:
...
internal class TranslationWorkflow(
    ...
    private suspend fun sendToWhisper(audioChunks: List<String>) {
        onLogUpdate(listOf("STEP 5: Sending to Groq Whisper API", "Endpoint: https://api.groq.com/openai/v1/audio/transcriptions", "Model: whisper-1"))
        for ((index, chunkPath) in audioChunks.withIndex()) {
            ...
            val response = GroqClient.apiService.transcribeAudio(authHeader, MultipartBody.Part.createFormData("file", chunkFile.name, chunkFile.asRequestBody(mimeType.toMediaTypeOrNull())), "whisper-1".toRequestBody("text/plain".toMediaTypeOrNull()))
            if (response.isSuccessful) { fullTranscript.append(response.body()?.text ?: "").append(" "); onLogUpdate(listOf("✓ Whisper response received")) } else { onLogUpdate(listOf("ERROR: Whisper API error: ${response.code()}")) }
            chunkFile.delete()
        }
        audioChunks.firstOrNull()?.let { File(it).takeIf { f -> f.exists() }?.delete() }
        onLogUpdate(listOf("Cleaned up temp audio file"))
    }
    ...
    private suspend fun sendToLLM(transcript: String) {
        ...
        onLogUpdate(listOf("STEP 7: Sending to Groq LLM", "Model: llama-3.3-70b-versatile"))
        val response = GroqClient.apiService.translateText(authHeader, ChatRequest(model = "llama-3.3-70b-versatile", messages = listOf(ChatMessage("system", "You are a translator. Translate the following English text to Bangla (Bengali). Only output the translation, nothing else."), ChatMessage("user", transcript))))
        val bangla = if (response.isSuccessful) response.body()?.choices?.firstOrNull()?.message?.content?.trim() ?: "" else ""
        onLogUpdate(listOf(if (bangla.isNotBlank()) "✓ LLM: ${bangla.take(50)}..." else "ERROR: LLM API error ${response.code()}"))
        onStepUpdate(baseSteps(6 to Triple(StepStatus.COMPLETED, bangla.take(50) + "...", null)))
    }
    ...
    private suspend fun createSubtitles() {
        ...
        val dialogues = createDialoguesFromTranscript(fullTranscript.toString().trim(), "")
